...
async function getTrip(id) {
  await connectDB();
  const trip = await SITrip.findById(id).lean();
  if (!trip) return null;
...
export default async function Page({ params }) {
  ...
  const trip_images = Array.from({ length: gallery_size }, (_, i) =>
    `/img/galleries/south_island${tripData.introduction.image}${i + 1}.webp`
  );
  ...
  return (
    <>
      <div>
        <GalleryHeader
          src={`/img/galleries/south_island${tripData.gallery_header.src}`}
