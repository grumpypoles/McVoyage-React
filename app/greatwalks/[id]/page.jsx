...
async function getTrip(id) {
  await connectDB();
  const trip = await GWTrip.findById(id).lean();
  if (!trip) return null;
  // lean() returns plain objects but _id is still a BSON ObjectId — serialise it
...
export default async function Page({ params }) {
  ...
  const trip_images = Array.from({ length: gallery_size }, (_, i) =>
    `/img/galleries/great_walks${tripData.introduction.image}${i + 1}.webp`
  );
  ...
  return (
    <>
      <div>
        <GalleryHeader
          src={`/img/galleries/great_walks${tripData.gallery_header.src}`}
